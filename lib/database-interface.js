...
class Database {
    ...
    */
    ...
    async deleteData(filter, dateFilter) {
        let deviceType = null
        switch(filter.type) {
            case "thermometer":
        ...
        let result = await this.deviceType.deleteMany({"token": filter.token}).where("timestamp").lt(dateFilter.start).gt(dateFilter.end)
        return result ? true : false
    }

    async deleteDataById(id, type) {
        let deviceType = null
        switch(type) {
            case "thermometer":
                deviceType = this.Thermometer
        ...
        let result = await this.deviceType.deleteOne({"_id": id})
        return result ? true : false
    }
}
