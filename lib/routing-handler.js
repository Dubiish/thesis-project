...
router.post("/devices", async (req, res) => {
    ...
    if(!req.body) {
        ...
        // Odpoved
        if(isNaN(result)) {
            res.json({"token": result}).status(201).end()
        } else {
...
router.get("/data/:token/:id", async(req, res) => {
...
router.put("/data/:token/:id", async (req, res) => {
    console.log("UPDATE >> Data")

    if(!req.body || !req.params.token || !req.params.id) {
        res.status(400).end()
    } else {
        let result = await dataHandler.updateData(req.body, req.params.token, req.params.id)
        res.status(result).end()
    }
})

router.delete("/data/:token", async (req, res) => {
    ...
    if(!token) {
        ...
        let response = await dataHandler.deleteData(filter, token)
        res.status(response).end()
    }
...
router.get("/lamp/:token", async(req, res) => {
    ...
    if(!req.params.token) {
        ...
        let response = await dataHandler.getLamp(req.params.token)
...
router.put("/lamp/:token", async (req, res) => {
