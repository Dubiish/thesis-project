...
class DataHandler {
    ...
    */
    ...
    async deleteData(filter) {
        ...
        let devices = null
        if(filter.token) {
            devices = await this.db.getDevices({"token": token})
            if(!devices.length) {
                return 404
            }
        } else if(filter.type) {
            devices = await this.db.getDevices({"type": type})
            if(!devices.length) {
                return 404
            }
        }
        ...
        if(devices.length) {
            for(const device of devices) {
                let result = await this.db.deleteData({"token": device.token, "type": device.type}, dateFilter)
                if(!result) {
                    return 404
                }
            }
            ...
            if(filter.id && filter.type) {
                let result = await this.db.deleteDataById(filter.id, filter.type)
                return result ? 204 : 500
            }
        }
